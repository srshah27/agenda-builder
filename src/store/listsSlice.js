...
const listsSlice = createSlice({
  ...
  reducers: {
    ...
    deleteAllliststtribute: (state, action) => {
      for (let i = 0; i < state.lists; i++) {
        state.lists[i].activityAttributes = state.lists[i].activityAttributes.filter(attr => attr !== action.payload);
      }
    },
      ...
      for (const key in newAttr) {
        if (Object.hasOwnProperty.call(newAttr, key)) {
          ...
          state.lists.forEach(list => {
            let index = list.activityAttributes.findIndex(attr => attr.id === attrId);
